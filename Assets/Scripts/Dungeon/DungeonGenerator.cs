...
namespace Dungeon
{
    ...
    {
        [SerializeField] 
        KeyCode regenerateKey = KeyCode.R;
        [SerializeField] 
        ...
        private int maxAttemptsTimes = 50;
        ...
        private void GenerateBlockeds()
        {
            foreach (var connector in transform.GetComponentsInChildren<Connector>())
            {
                if (!connector.isConnected)
                {
                    ...
                    GameObject blockedGo = Instantiate(blockedPrefabs[blockedIndex], connector.transform.position, connector.transform.rotation, connector.transform);
        ...
        private void GenerateDoors()
        {
            foreach (var connector in transform.GetComponentsInChildren<Connector>())
            {
                if (connector.isConnected)
                {
                    ...
                    {
                        Collider[] colliders = Physics.OverlapBox(connector.transform.position + connector.transform.up * 0.5f, 
                            new Vector3(connector.size.x, 1f, connector.size.x), 
                            Quaternion.identity, 
                        ...
                        if (colliders.Length == 0)
                        {
                            ...
                            GameObject doorGo = Instantiate(doorPrefabs[doorIndex], connector.transform.position, connector.transform.rotation, connector.transform);
                            doorGo.name = doorPrefabs[doorIndex].name;
                        }
                    }
                }
            }
